...
class Utility extends Model
{
    ...
    public static function settings()
    {
        if (self::$fetchsettings === null) {
            ...
            if (\Auth::check()) {
                $data = $data->where('created_by', '=', \Auth::user()->creatorId())->where('store_id', \Auth::user()->current_store);

                if ($data->count() === 0) {
                    $data = DB::table('settings')->where('created_by', '=', 1)->where('store_id', \Auth::user()->current_store);
                }
...
Order Date:" . ' ' . self::dateFormat($order->created_at));
    ...
    public static function getSMTPDetails($user_id)
    {
        ...
        if ($store) {
            config([
                'mail.default'                   => isset($store['mail_driver'])       ? $store['mail_driver']       : '',
                'mail.mailers.smtp.host'         => isset($store['mail_host'])         ? $store['mail_host']         : '',
                'mail.mailers.smtp.port'         => isset($store['mail_port'])         ? $store['mail_port']         : '',
                'mail.mailers.smtp.encryption'   => isset($store['mail_encryption'])   ? $store['mail_encryption']   : '',
                'mail.mailers.smtp.username'     => isset($store['mail_username'])     ? $store['mail_username']     : '',
                'mail.mailers.smtp.password'     => isset($store['mail_password'])     ? $store['mail_password']     : '',
                'mail.from.address'              => isset($store['mail_from_address']) ? $store['mail_from_address'] : '',
                'mail.from.name'                 => isset($store['mail_from_name'])    ? $store['mail_from_name']    : '',
            ]);
    ...
    public static function addNewData()
    {
        ...
        $companyRole = Role::where('name', 'LIKE', 'owner')->first();

        $ownerPermissions   = $companyRole->getPermissionNames()->toArray();
        $ownerNewPermission = [
            'Manage Language',
            'Create Language',
            'Delete Language'
