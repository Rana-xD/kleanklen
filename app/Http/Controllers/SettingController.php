...
class SettingController extends Controller
{
    ...
    public function index()
    {
        if(\Auth::user()->can('Manage Settings')){
            ...
            if ($store_settings) {
                ...
                return view('settings.index', compact('settings','timezones', 'store_settings', 'serverIp', 'subdomain_name', 'subdomain_Ip', 'subdomainPointing','domainip', 'domainPointing', 'store_payment_setting','pwa_data','PixelFields'));
            } else {
    ...
    public function saveBusinessSettings(Request $request)
    {
        ...
        if (\Auth::user()->type == 'super admin') {
            ...
            if (!empty($request->title_text) || !empty($request->color) || !empty($request->cust_theme_bg) || !empty($request->cust_darklayout) || !empty($request->footer_text) || !empty($request->default_language) || !empty($request->display_landing_page)) {
                $post = $request->all();
            ...
            if (!empty($request->title_text) || !empty($request->color) || !empty($request->cust_theme_bg) || !empty($request->cust_darklayout) || !empty($request->footer_text) || !empty($request->display_landing_page)) {
    ...
    public function recaptchaSettingStore(Request $request)
    {
        ...
        foreach ($post as $key => $data) {
            ...
            \DB::insert(
                'insert into settings (`value`, `name`, `store_id`,`created_by`) values (?, ?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`) ', $arr
            );
    ...
    public function chatgptkey(Request $request){
        ...
        if (!empty($request->chatgpt_key)) {
            ...
            $enable_chatgpt = $request->has('enable_chatgpt') ? $request->enable_chatgpt : 'off';
            $post['enable_chatgpt'] = $enable_chatgpt;
