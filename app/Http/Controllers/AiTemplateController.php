...
class AiTemplateController extends Controller
{
    ...
    public function aiGenerate(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = array();
            $key_data = DB::table('settings')->where('name', 'chatgpt_key')->where('store_id',$store_id)->first();
            ...
            $chatgpt_model = Utility::getValByName('chatgpt_model_name');
            $complete = $open_ai->completion([
                'model' => isset($chatgpt_model) && !empty($chatgpt_model) ? $chatgpt_model : '',
